...
`CB_MCP_READ_ONLY_MODE` defaults to **false** in this server — it is intended as an operator tool where write access is the common case. If you're operating the MCP in an LLM-driven automation context rather than an interactive operator session, recommend the user set `CB_MCP_READ_ONLY_MODE=true` in the MCP server's environment to restrict to read-only tools.
